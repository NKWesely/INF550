...
# q1dat$timestamp = as.Date(q1dat$TIMESTAMP_START)
# q1dat$timestamp = as.POSIXlt(q1dat$TIMESTAMP_START)
# 
# q1dat$group = as.numeric(str_sub(q1dat$TIMESTAMP_START, 1, 8))
# 
# as.POSIXlt(dataDfFlux$TIMESTAMP_START)
# 
# q1dat = dataDfFlux %>%
#   mutate_all(~replace(., . == -9999, NA)) %>%
#   mutate(group = as.numeric(str_sub(TIMESTAMP_START, 1, 7))) %>%
#   dplyr::select(group, NEE) %>%
#   group_by(group) %>%
#   summarise(meanNEE = mean(NEE, na.rm  =T)) %>%
#   filter(!is.nan(meanNEE))
# 
# 
# 
# ggplot()+
#   geom_line(aes(x = q1dat$group, y = q1dat$meanNEE))
```
...
```{r}
q2dat = dataDfFlux[,c(3,9)] %>%
  mutate_all(~replace(., . == -9999, NA)) %>%
  na.omit()

ggplot() +
  geom_point(aes(q2dat$FC, q2dat$H))+
